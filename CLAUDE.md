...
`Routines`（Anthropicのクラウド上で動く、セッション不要の永続スケジュール）で
Issue駆動のループを構築してある。`/loop` や `CronCreate` はセッションスコープ
（セッションを開いていないと発火しない）なので使っていない。

**運用：**

1. 人間が GitHub Issue に `auto-ok` ラベルを付けて、自走対象として許可する
2. 日次1回、Routineが `auto-ok` の付いたIssueを1件だけ拾い（無ければ何もしない）、
   実装してPRを作る。着手したら `auto-ok` を外し `auto-in-progress` を付ける
3. **PRは既存の壁（保護パス判定・CI・ミューテーションテスト）をそのまま通る。**
   `src/` だけを触っていてCIが通れば、人間が一度も見ないまま自動マージされ得る
   ——これがPhase 5の狙いそのもの。保護パスを含む場合は他のPRと同様に人間の
   マージ待ちになる
4. AIはPRをマージしない、という制約はRoutine実行時も変わらないはず——ただし
   `.claude/settings.json` の `deny`（`gh pr merge` 等）がRoutine実行時にも
   効くかは、公式ドキュメントで確証が取れていない。**プロンプトの自己申告だけに
   頼らず、有効化前に実際にdenyされるかを検証する**（手順は
   `.claude/routines/daily-issue-loop.md` に記載）。これは「壁はプロンプトの
   指示ではなくGitHub側の機械的な仕組みであるべき」という一貫方針に基づく
   ——push前レビューで実際にCodexに指摘された点

実プロンプトは Anthropic 側に保存されており、`.claude/routines/daily-issue-loop.md`
はその参照用コピー（実行時に読まれるわけではない）。プロンプトを変えたら両方を
同じ内容に更新する。

**現在は一時停止状態。** 有効化は人間が行う。**denyの実地検証が済むまで有効化しない。**
検証済み・有効化後は `/dev-status` で保護パス比率・PR一覧・codex-reviewの利用実績の
推移を定期的に確認する——これが human-on-the-loop の実体で、各PRを個別承認する
わけではない。
